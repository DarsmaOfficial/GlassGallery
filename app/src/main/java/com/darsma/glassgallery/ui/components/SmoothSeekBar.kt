...
fun SmoothSeekBar(
    ...
    val activeAnimated = animateColorAsState(
    ...
    val waveMotionActive = !dragging && isPlaying
    val waveAmplitude = animateFloatAsState(
        targetValue   = if (waveMotionActive) 1f else 0f,
        animationSpec = Motion.gentle(),
        label = "wave-amp",
    )
    val wavePhase: State<Float> = if (waveMotionActive) {
    ...
    Canvas(
        ...
        val th = trackHeight.value.toPx()
        val r  = thumbRadius.value.toPx()
        val usableW = size.width - r * 2f
        val thumbX  = r + usableW * animatedProgress.value
        val activeColorValue = activeAnimated.value
        ...
        val ampPx = 2.6.dp.toPx() * waveAmplitude.value
        if (ampPx > 0.4f) {
            ...
            while (x <= end) {
                ...
                val y = cy + sin((x / waveLen) * 2f * PI.toFloat() - wavePhase.value) * ampPx * fade
        ...
        if (glowAlpha.value > 0f) {
            drawCircle(
                color  = activeColorValue.copy(alpha = glowAlpha.value),
                radius = r * 2.2f,
